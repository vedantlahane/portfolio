import React, { useState, useEffect } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { usePortfolio } from '../context/PortfolioContext';

export default function Preloader({ onComplete }) {
    const [stage, setStage] = useState('loading'); // 'loading' | 'sliding' | 'done'
    const { loading } = usePortfolio();

    useEffect(() => {
        // We need to wait for BOTH the minimum animation time (1.5s) AND the API data to finish loading
        let minTimeElapsed = false;
        let dataLoaded = !loading;

        const timer1 = setTimeout(() => {
            minTimeElapsed = true;
            if (dataLoaded) {
                setStage('sliding');
            }
        }, 1500);

        if (!loading) {
            dataLoaded = true;
            if (minTimeElapsed) {
                setStage('sliding');
            }
        }

        return () => clearTimeout(timer1);
    }, [loading]);

    useEffect(() => {
        if (stage === 'sliding') {
            const timer2 = setTimeout(() => {
                setStage('done');
                if (onComplete) onComplete();
            }, 800); // 800ms for slide out
            return () => clearTimeout(timer2);
        }
    }, [stage, onComplete]);

    if (stage === 'done') return null;

    return (
        <AnimatePresence>
            <motion.div
                className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-white dark:bg-black pointer-events-none transition-colors duration-300"
                initial={{ originY: 0 }}
                animate={stage === 'sliding' ? { scaleY: 0 } : { scaleY: 1 }}
                transition={{ duration: 0.5, ease: [0.76, 0, 0.24, 1] }}
            >
                <div
                    className="flex flex-col items-center justify-center"
                    style={{ opacity: stage === 'sliding' ? 0 : 1, transition: 'opacity 0.2s' }}
                >
                    <motion.div
                        className="w-12 h-12 border-2 border-gray-900 dark:border-white mb-8"
                        animate={{ rotate: [45, 135, 225, 315] }}
                        transition={{ duration: 2, ease: "linear", repeat: Infinity }}
                    />
                    <motion.div
                        className="font-mono text-[10px] tracking-[0.3em] text-gray-400 dark:text-neutral-400 font-medium ml-1"
                        animate={{ opacity: [0.3, 1, 0.3] }}
                        transition={{ duration: 1.5, ease: "easeInOut", repeat: Infinity }}
                    >
                        {loading ? "F E T C H I N G   R E P O" : "L O A D I N G"}
                    </motion.div>
                </div>
            </motion.div>
        </AnimatePresence>
    );
}
